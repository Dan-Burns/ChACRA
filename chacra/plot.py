...
def plot_difference_of_roots(
    ...
    if cpca._permutated_explained_variance is None:
        ...
        p_val = (
            np.sum(
                np.abs(np.diff(variance, axis=1, prepend=0))
                > np.abs(np.diff(original_variance, prepend=0)),
                axis=0,
            )
            / cpca._N_permutations
        )
        ax.hlines(0.05, xmin=0, xmax=n_pcs, color=cutoff_color, zorder=1)
        ax.scatter(
            [f"{i+1}" for i in range(n_pcs)],
            p_val[:n_pcs],
            color=dot_color,
...
def plot_chacras(
    ...
    if temps is None and contacts is not None:
        print(
            "Using the axis labels from the contact data as temperature labels. "
        ...
        print(
            "No temperatures or contact frequency data provided to obtain temperatures from. X-axis corresponds to system id."
        )
...
def biplots(
    ...
    fig, axs = plt.subplots(
        n_rows, 4, figsize=(15, 3 * n_rows), constrained_layout=True
    )
    for i, combo in enumerate(combos):
        ...
        a_grad = get_color_gradient(
            "#ffffff00", colors[pc_a - 1], 100, return_rgb=True, alpha=True
        )
        b_grad = get_color_gradient(
            "#ffffff00", colors[pc_b - 1], 100, return_rgb=True, alpha=True
        )
        ...
        if label_top is not None:
            ...
            top_labels.extend(
                list(cpca.sorted_norm_loadings(pc_b).index[:label_top])
            )

            labels = [
                label if label in top_labels else None for label in labels
            ]
