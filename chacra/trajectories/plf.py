...
getcontacts_params = {
    "Anionic": {
        "distance": 4.0,  # considered 4.5 by prolif
    },
    "CationPi": {
        "distance": 6.0,  # 4.5 for prolif
        "angle": (0, 60),
    },  # 0 to 30 for prolif
    "Cationic": {
        "distance": 4.0,
    ...
    "FaceToFace": {
        "distance": 7.0,  # 5.5 for prolif
        "plane_angle": (0, 30),
        "normal_to_centroid_angle": (0, 45),
    },
    "HydrophobicVdW": {
        "threshold": 0.5,
    },  # Original 'Hydrophobic' is "distance":4.0
    "HBAcceptor": {"DHA_angle": (110, 180)},  # <70 from AHD
    "HBDonor": {"DHA_angle": (110, 180)},
    "PiCation": {"distance": 6.0, "angle": (0, 60)},
    "VdWContact": {
        "tolerance": 0.5,
    },
}
        ...
        if lig_matches and prot_matches:
            ...
            for lig_match, prot_match in product(lig_matches, prot_matches):
                ...
                try:
                    ...
                    vdw = (
                        self.vdwradii[lig] + self.vdwradii[res] + self.tolerance
                    )
...
def get_prolif_freqs(df):
    ...
    for res in reslabels:
        ...
        cols = set(
            [
                a
                for tup in df.xs(res, level="ligand", axis=1).columns
                for a in tup[:1]
            ]
        )
        for col in cols:
            ...
            if contact in data.keys():
                ...
                bool_contacts = df.xs(res, level="ligand", axis=1).xs(
                    col, level="protein", axis=1
                )

                result = (
                    len(bool_contacts.loc[bool_contacts.any(axis=1) == True])
                    / n_frames
                )
