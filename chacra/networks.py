...
def get_communities(
    G: nx.Graph, n_communities: int = 5, removed_edges: bool = False
):
    """
    # visualize with
...
def get_betweenness_centrality(G: nx.Graph, weight: str = "inverse"):
    """
...
def pc_network(
    ...
    path = [start_contact]
    next_contact = path[-1]
    while (len(path) < n_contacts) and (
        end_contact is None or end_contact not in path
    ):

        resa, resb = next_contact.split("-")
        # all the columns that involve either residue
        cols = list(
            contact_data.norm_loadings.filter(
                regex=f"{resa}|{resb}", axis=0
            ).index
        )
