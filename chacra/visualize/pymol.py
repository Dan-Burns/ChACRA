...
def get_contact_data(
    ...
    for contact in contact_list:
        chaina, resna, resia, chainb, resnb, resib = re.split(":|-", contact)
        ...
        score = contactPCA.get_top_score(
            contact, pc_range=(pc_range[0], pc_range[1])
        )
        ...
        data[contact]["slope"] = get_slope(
            cdf,
            contact,
            temp_range=(slope_range[0], min(slope_range[1], cdf.shape[0])),
        )

    lowest_score = sorted(data.items(), key=lambda t: t[1]["loading_score"])[0][
        1
    ]["loading_score"]
    interpolator = get_variance_to_sphere_scale_interpolator(
        lowest_score,
    ...
    for contact in data.keys():
        ...
        data[contact]["sphere_scale"] = (
            1
            if variable_sphere_scale is False
            else interpolator(data[contact]["loading_score"])
        )
    ...
    result = collections.OrderedDict(
        sorted(data.items(), key=lambda t: t[1]["loading_score"])
    )
    return result
    ...
    with open(output_file, "w") as f:
        ...
        for contact, data in contact_data.items():
            ...
            if ca_only == True:
                ...
                f.write(
                    f"select {contact}, (chain {data['chaina']} and resi {data['resia']} "
            ...
            # draw the line
            f.write(
                f"distance {contact}-line, (chain {data['chaina']} and resi {data['resia']} and name CA), "
            ...
            if "sphere_scale" in data.keys():
                f.write(
                    f"set sphere_scale, {data['sphere_scale']}, {contact} \n"
                )
...
def get_slope(df, contact, temp_range=(0, 7)):
    ...
    return linregress(
        df[contact].iloc[temp_range[0] : temp_range[1]].index,
        df[contact].iloc[temp_range[0] : temp_range[1]],
    ).slope
...
def get_variance_to_sphere_scale_interpolator(
    ...
    interpolator = interp1d(
        [min_loading_score, 1], [min_sphere_scale, max_sphere_scale]
    )

    return interpolator


def pymol_averaged_chacras_to_all_subunits(
    mapped_contacts,
    pymol_data,
    output,
):
    ...
    for averaged_contact_name in pymol_data:
        full_protein_pymol_data[averaged_contact_name] = pymol_data[
            averaged_contact_name
        ]
        for replicated_name in mapped_contacts[averaged_contact_name]:
            if replicated_name != averaged_contact_name:
                ...
                full_protein_pymol_data[replicated_name] = {
                    ...
                    "loading_score": pymol_data[averaged_contact_name][
                        "loading_score"
                    ],
                    "color": pymol_data[averaged_contact_name]["color"],
                    "sphere_scale": pymol_data[averaged_contact_name][
                        "sphere_scale"
                    ],
...
def nx_edges_to_pymol(
    ...
    with open(output, "w") as file:
        ...
        for edge in edges.keys():
            ...
            name = (
                f"{edge1_chain}{edge2_chain}-{edge1_resid}-{edge2_resid}-line"
            )
            # draw the line
            dist_string = (
                f"distance {name}, (chain {edge1_chain} and resi {edge1_resid} and name CA), "
            ...
            if variable_width:
                ...
                edge_size = (
                    ((edge_weight - min_weight) / (norm_denominator))
                    * size_coef
                ) + min_width
...
def nx_to_pymol(output, sel_name, res_list, iteration):
    ...
    with open(output, "a") as file:
        ...
        color_string = (
            "color " + colors[iteration] + ", " + "community_" + str(iteration)
        )
...
def label_distance(chaina, resa, chainb, resb):
    string = (
        f"distance {chaina}:{resa}-{chainb}-{resb}, "
        ...
        for i, res in enumerate(res_list):
            ...
            if i != len(res_list) - 1:
                ...
                if i > 0:
                    ...
                    dist_string = label_distance(
                        chain, resid, prev_chain, prev_resi
                    )
                    color_distance = (
                        f"color blue, {chain}:{resid}-"
...
def node_centrality_gradient(
    output="./colors.pml", cmap_name="plasma", node_centralities=None
):
    """
...
def coordinate_color_gradient(
    output="./colors.pml", structure=None, cmap_name="plasma", coord="y"
):
    """
            ...
            if chain_id == seg_id:
                ...
                for i, resi in enumerate(list(coordinates.keys())):
                    ...
                    vector_magnitudes[resi] = np.linalg.norm(
                        [centered_array[i][0], centered_array[i][1]]
                    )
