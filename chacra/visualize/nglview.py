...
def get_contact_indices(contact, u, ca_only=True):
    ...
    for res in ['a','b']:
        if ca_only:
            indices.append(np.where((u.atoms.resnums == int(c[f'resid{res}'])) & (u.atoms.segids == c[f'chain{res}']) & (u.atoms.names == 'CA'))[0][0])
        else:
            indices.append(np.where((u.residues.resids == int(c[f'resid{res}'])) & (u.residues.segids == c[f'chain{res}']))[0][0]) 
...
def draw_line(contact, u, view, width=0.3, arrows=False, contact_data=None, variable_width=False, width_coef=1.3, exp=2, color=None):
    '''
...
class Visualizer:
    ...
    '''
    def __init__(self, structure, cpca, cont, protein_color='silver', bkgrnd='black'):
        ...
        self.view.add_representation('cartoon', selection='protein', color=self.protein_color)

    def view(self):
        return self.view
    
    def show_chacras(self, pc_range, cutoff=0.6, ca_spheres=True,  arrows=True, sphere_scale=0.8, 
                     variable_line_width=False, line_width=0.3, clear_representation=False):
        ...
        for i in range(min_pc,max_pc+1):
            top_contacts.extend((self.cpca.sorted_norm_loadings(i).loc[
                            self.cpca.sorted_norm_loadings(i)[f'PC{i}'] > cutoff]).index)
            
        top_contacts = list(set(top_contacts))
        contact_data = get_contact_data(top_contacts, self.cont, self.cpca, pc_range=pc_range)
        ...
        for contact in contact_data:
            if ca_spheres == True:
                ...
                ca_colors[atom_indices[0]] = chacra_colors[contact_data[contact]['top_pc']-1]
                ca_colors[atom_indices[1]] = chacra_colors[contact_data[contact]['top_pc']-1]
            
            draw_line(contact, self.u, self.view, width=line_width, arrows=arrows, 
                      variable_width=variable_line_width, width_coef=line_width, contact_data=contact_data)
        ...
        if ca_spheres == True:
            ...
            sphere_colors = {value: ','.join([str(atom_index) for atom_index in ca_colors.keys() if ca_colors[atom_index]==value])  
