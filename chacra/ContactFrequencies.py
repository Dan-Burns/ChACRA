...
def make_contact_dataframe(
    freq_files: str | os.PathLike | list, temps: list = None
) -> pd.DataFrame:
    """
...
class ContactFrequencies:
            ...
            if temp_progression == "linear":
                temps = [
                    ...
                    for i in np.linspace(
                        min_max_temp[0], min_max_temp[1], len(self.freqs)
                    )
                ]
            elif temp_progression == "geometric":
                temps = [
                    int(i)
                    for i in np.geomspace(
                        min_max_temp[0], min_max_temp[1], len(self.freqs)
                    )
                ]
            mapper = {key: temp for key, temp in zip(self.freqs.index, temps)}
    ...
    def find_correlated_contacts(self, contact, inverse=True):
        ...
        if inverse == True:
            out = cor.filter(regex=rf"{a}|{b}", axis=0).sort_values(
                by=contact, ascending=True
            )
        else:
            out = cor.filter(regex=rf"{a}|{b}", axis=0).sort_values(
                by=contact, ascending=False
            )
    ...
    def get_edges(
        ...
        for contact in self.freqs.columns:
            ...
            if weights == True:
                ...
                all_contacts.append(
                    (partners["resa"], partners["resb"], weight)
                )
    ...
    def get_all_residues(self):
        ...
        return list(
            set(
                [
                    a
                    for cont in self.freqs.columns
                    for a in split_id(cont).values()
                ]
            )
        )
    ...
    def exclude_neighbors(self, n_neighbors=1):
        ...
        for contact in self.freqs.columns:
            ...
            if id_dict["chaina"] != id_dict["chainb"]:
                ...
                if (
                    np.abs(int(id_dict["resida"]) - int(id_dict["residb"]))
                    > n_neighbors
                ):
    ...
    def exclude_below(self, min_frequency=0.05, row_range=None):
        ...
        if row_range:
            return self.freqs[
                (
                    self.freqs.iloc[row_range[0] : row_range[1]].max()
                    > min_frequency
                ).index[
                    self.freqs.iloc[row_range[0] : row_range[1]].max()
                    > min_frequency
            ...
            return self.freqs[
                (self.freqs.max() > min_frequency).index[
    ...
    def exclude_above(self, max_frequency=0.98):
        ...
        """
        return self.freqs[
            (self.freqs.min() < max_frequency).index[
    ...
    def to_heatmap(self, output_format="mean", pc=None, row=None):
        ...
        if row is not None:
            if row in self.freqs.index:
                ...
                print(
                    f"Row {row} is not in the frequency data index. "
...
def de_correlate(a):
    ...
    ]  # index by independently randomized rows
    return out
...
def run_permuted_pca(vals, n_permutations):
    ...
    pca = PCA()
    xvrs = [
        pca.fit(de_correlate(vals)).explained_variance_ratio_
...
class ContactPCA:
    ...
    def __init__(
        ...
        self.loadings = pd.DataFrame(
            ...
            columns=[
                "PC" + str(i + 1)
                for i in range(np.shape(pca.explained_variance_ratio_)[0])
            ],
        ...
        if (
            linregress(
                range(self.loadings.shape[1]), self._transform[:, 0]
            ).slope
            > 0
        ):
    ...
    def get_chacra_center(self, pc, cutoff=0.6, absolute=True):
        ...
        chacra_centers = (
            self.sorted_norm_loadings(pc)
            .loc[(self.sorted_norm_loadings(pc)[f"PC{pc}"] >= cutoff)]
            .index
        )
        if absolute == True:
    ...
    def permuted_pca(self, N_permutations=500, n_jobs=None):
        ...
        n_permutations_list = [
            base_n + (1 if i < remainder else 0) for i in range(n_jobs)
        ]

        results = Parallel(n_jobs=n_jobs, backend="loky")(
            delayed(run_permuted_pca)(df_values, n)
            for n in tqdm.tqdm(n_permutations_list)
        )
        ...
        real_diffs = np.abs(
            np.diff(self.pca.explained_variance_ratio_, prepend=0)
        )
    ...
    def to_pymol(
        ...
        if reconstruct_from_averaged is True and self.structure is None:
            print(
                "You must assign a structure file to cpca.structure before "
                "the contacts can be reconstructed on all subunits."
            )

        elif reconstruct_from_averaged is True and original_contacts is None:
            print(
                "Provide the original (all contacts before averaging) "
                "contacts data if you want to reconstruct the full structure."
            )
        elif (
            reconstruct_from_averaged is True and representative_chains is None
        ):
            print(
                "Provide a list of the representative chain ids use for "
                "averaging."
            )
        ...
        if reconstruct_from_averaged is True:
            ...
            mapped_contacts = everything_from_averaged(
                self.freqs[top_contacts], u, representative_chains, as_map=True
            )
            ...
            pymol_averaged_chacras_to_all_subunits(
                mapped_contacts, pymol_data, output
            )
...
class CombinedChacra:
    ...
    def __init__(self, data_dict):
        ...
        self.mappers = {
            key: {col: f"{key}_{col}" for col in data_dict[key].columns}
        ...
        self.shared_contacts = list(
            multi_intersection(
                [(list(data.keys())) for data in self.mappers.values()]
            )
        )
        self.not_shared_contacts = {
            key: [
                contact
                for contact in self.mappers[key].keys()
        ...
        for contact in self.shared_contacts:
            diff = np.abs(
                self.original_data[self.names[0]][contact].values
        ...
        for name in self.not_shared_contacts:
            for contact in self.not_shared_contacts[name]:
                difference_data[contact] = self.original_data[name][
                    contact
                ].values
    ...
    def get_top_changes(self, cutoff, min_loading_dif=0.2, pc_range=None):
        ...
        for pc in pcs:
            ...
            for contact in self.shared_contacts:
                ...
                if (
                    contacta in above_cutoff and contactb not in above_cutoff
                ) and (
                    (
                    ...
                    different[pc].append((contacta, contactb))

                elif (
                    contactb in above_cutoff and contacta not in above_cutoff
                ) and (
                    (
                        self.combined.cpca.norm_loadings[f"PC{pc}"].loc[
                            contactb
                    ...
                    different[pc].append((contactb, contacta))
    ...
    def get_changes(self, stdev_min=0.0, stdev_max=0.02, mean_dif=0.2):
        ...
        for contact in self.shared_contacts:
            if (
                (
                    stdev_min
                    < self.original_data[self.names[0]][contact].std()
                    < stdev_max
                )
                and (
                    stdev_min
                    < self.original_data[self.names[1]][contact].std()
                    < stdev_max
                )
            ) and (
                np.abs(
                    self.original_data[self.names[0]][contact].mean()
                    - self.original_data[self.names[1]][contact].mean()
                )
                > mean_dif
            ):
    ...
    def separate_ensemble_loadings(self):
        ...
        self.separated_cpca = {
            name: ContactPCA(self.original_data[name], significance_test=False)
        ...
        for name in self.names:
            ...
            self.separated_cpca[name].loadings = (
                self.combined.cpca.loadings.filter(
                    like=name, axis=id_axis
                ).rename(self.reverse_mappers[name], axis=0)
            )
